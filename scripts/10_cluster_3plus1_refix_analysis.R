# =============================================
# Script: 09_cluster_3plus1_refix_analysis.R
# Purpose: Clustering with prog_z, reread_z, lookback_z + refix
# =============================================
...
# 2. Select features
features <- df %>% select(prog_z, reread_z, lookback_z, refix)
...
df_clean$Cluster_3P1_refix <- factor(k3_model$cluster)
...
# 6. Compute centroids
cluster_centroids <- df_clean %>% group_by(Cluster_3P1_refix) %>%
...
rownames(mahal_dist) <- paste0("C", 1:3, "_3+refix")
rownames(cosine_dist) <- paste0("C", 1:3, "_3+refix")
colnames(mahal_dist) <- colnames(cosine_dist) <- paste0("GG", 1:3)

# 8. Save distances
write.csv(mahal_dist, "C:/Users/farha/ANLP_project/Copnew20/outputs/plots/mahalanobis_3plus_refix.csv")
write.csv(cosine_dist, "C:/Users/farha/ANLP_project/Copnew20/outputs/plots/cosine_3plus_refix.csv")

# 9. PCA plots: PC1–PC2, PC1–PC3, PC2–PC3
pca <- prcomp(features_clean, center = TRUE, scale. = TRUE)
pca_df <- as.data.frame(pca$x)
pca_df$Cluster <- df_clean$Cluster_3P1_refix
...
p1 <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Cluster)) +
  ...
  labs(title = "(a) PC1 vs PC2: 3 + refix", x = "PC1", y = "PC2") +
  scale_color_brewer(palette = "Dark2")

ggsave("C:/Users/farha/ANLP_project/Copnew20/outputs/plots/pca_3plus_refix_PC1_PC2.png", p1, width = 6, height = 5, dpi = 300)
...
p2 <- ggplot(pca_df, aes(x = PC1, y = PC3, color = Cluster)) +
  ...
  stat_ellipse(type = "norm", linetype = "dashed", size = 1) +
  theme_minimal(base_size = 14) +
  labs(title = "(b) PC1 vs PC3: 3 + refix", x = "PC1", y = "PC3") +
  scale_color_brewer(palette = "Dark2")

ggsave("C:/Users/farha/ANLP_project/Copnew20/outputs/plots/pca_3plus_refix_PC1_PC3.png", p2, width = 6, height = 5, dpi = 300)
...
p3 <- ggplot(pca_df, aes(x = PC2, y = PC3, color = Cluster)) +
  ...
  stat_ellipse(type = "norm", linetype = "dashed", size = 1) +
  theme_minimal(base_size = 14) +
  labs(title = "(c) PC2 vs PC3: 3 + refix", x = "PC2", y = "PC3") +
  scale_color_brewer(palette = "Dark2")

ggsave("C:/Users/farha/ANLP_project/Copnew20/outputs/plots/pca_3plus_refix_PC2_PC3.png", p3, width = 6, height = 5, dpi = 300)

message("✅ PCA plots with ellipses saved for 3 + refix")
