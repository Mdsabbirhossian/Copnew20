# ====================================================================
# Script: plot_figure_cluster_ggroup_3plus_skip.R
...
mahal_file <- file.path(base_path, "mahalanobis_3plus_skip.csv")
cosine_file <- file.path(base_path, "cosine_3plus_skip.csv")
...
p1 <- ggplot(mahal_long, aes(x = GGroup, y = Cluster, fill = Dist)) +
  ...
  labs(
    title = "(a) Mahalanobis Distance: 3 + skip",
...
p2 <- ggplot(cosine_long, aes(x = GGroup, y = Cluster, fill = Dist)) +
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red",
                       ...
                       limits = c(0, 2)) +
  labs(
    title = "(b) Cosine Distance: 3 + skip",
    x = "G-group Centroid (G1–G3)",
    y = "Cluster Centroid (C1–C3)"
  ) +
  theme_minimal(base_size = 13)
...
annotated_plot <- annotate_figure(
  combined_plot,
  top = text_grob("Figure: Cluster–G-group Distance Comparison (3 Features + Skip)", face = "bold", size = 14)
)

ggsave(file.path(base_path, "figure_cluster_ggroup_3plus_skip.png"),
       plot = annotated_plot, width = 14, height = 6, dpi = 300)

message("✅ Distance heatmap for 3+skip saved.")
